...
def jsonauto(Json: dict, Action: str, PATH: Path):
    ...
    with FileLock:
        if PATH.stem+PATH.suffix == "config.json":
            DefaultJSON = {"Root": None, "Admin": [], "BotQQ": None, "NotAllowUser": [], "BadWords": [
            ], "@Me": None, "AdminGroup": [], "Server": {"AcceptPort": 5120, "PostIP": "127.0.0.1:5700", "AccessKey": None}}
            if not PATH.exists():
            ...
            if Action == "WRITE":
                ...
                if set(Res.keys()) != set(DefaultJSON.keys()):
                    Res.update(
                        {key: DefaultJSON[key] for key in DefaultJSON.keys() if key not in Res})
                return Res
            elif Action == "TEXT":
                with open(PATH, "rt", encoding="utf-8") as file:
                    Res: dict = json.load(file)
            ...
            if not PATH.exists():
                urls = ("https://cdn.jsdelivr.net/gh/CoolPlayLin/CoolPlayLin-Bot@main/src/database/API.json",
                        "https://fastly.jsdelivr.net/gh/CoolPlayLin/CoolPlayLin-Bot@main/src/database/API.json", "https://gitee.com/coolplaylin/CoolPlayLin-Bot/raw/main/src/database/API.json")
                for each in urls:
                    try:
                        with open(PATH, "w+", encoding="utf-8") as f:
            ...
            if not PATH.exists():
                urls = ("https://cdn.jsdelivr.net/gh/CoolPlayLin/CoolPlayLin-Bot@main/src/database/db.json",
                        "https://fastly.jsdelivr.net/gh/CoolPlayLin/CoolPlayLin-Bot@main/src/database/db.json", "https://gitee.com/coolplaylin/CoolPlayLin-Bot/raw/main/src/database/db.json")
    ...
    def error(self, msg: str) -> bool:
        with self.FileLock:
            with open(self.PATH, "a", encoding="utf-8") as f:
                f.write("{} Error: {}\n".format(
                    time.strftime(r"%Y-%m-%d %H:%M:%S"), msg))
                return True
...
class Cache:
    ...
    def write_res(self, objects: requests.Response, prefix: str) -> Path:
        _path = self.PATH / \
            "{}{}.jpg".format(prefix, random.randint(1, 100000000))
        while _path.exists():
            _path = self.PATH / \
                "{}{}.jpg".format(prefix, random.randint(1, 100000000))
    ...
    def save_text(self, text: str, suffix: str, prefix: str = "") -> Path:
        _path = self.PATH / \
            "{}{}.{}".format(prefix, random.randint(1, 100000000), suffix)
        while _path.exists():
            _path = self.PATH / \
                "{}{}.{}".format(prefix, random.randint(1, 100000000), suffix)
